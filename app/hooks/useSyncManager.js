...
export function useSyncManager({ showToast, refreshAllRef, setTempSeconds, setFundTagRecords }) {
  ...
  function getComparablePayload(payload) {
    ...
    return JSON.stringify({
      ...
      portfolios: isArray(payload.portfolios) ? payload.portfolios : [],
      portfolioHoldings: isArray(payload.portfolioHoldings) ? payload.portfolioHoldings : [],
      portfolioTransactions: isArray(payload.portfolioTransactions) ? payload.portfolioTransactions : [],
      portfolioPrincipalRecords: isArray(payload.portfolioPrincipalRecords) ? payload.portfolioPrincipalRecords : [],
      portfolioSnapshots: isArray(payload.portfolioSnapshots) ? payload.portfolioSnapshots : [],
      portfolioBacktests: isArray(payload.portfolioBacktests) ? payload.portfolioBacktests : [],
      portfolioSettings: isPlainObject(payload.portfolioSettings) ? payload.portfolioSettings : {},
      portfolioSchemaVersion: Number(payload.portfolioSchemaVersion) || 1
    });
  }
  ...
  const collectLocalPayload = (keys = null) => {
    try {
      ...
      Object.entries(PORTFOLIO_SYNC_DEFAULTS).forEach(([key, fallback]) => {
        if (!keys || keys.has(key)) {
          all[key] = storageStore.getItem(key, fallback);
        }
      });
      ...
      if (!keys) {
        ...
        return {
          ...
          portfolios: isArray(all.portfolios) ? all.portfolios : [],
          portfolioHoldings: isArray(all.portfolioHoldings) ? all.portfolioHoldings : [],
          portfolioTransactions: isArray(all.portfolioTransactions) ? all.portfolioTransactions : [],
          portfolioPrincipalRecords: isArray(all.portfolioPrincipalRecords) ? all.portfolioPrincipalRecords : [],
          portfolioSnapshots: isArray(all.portfolioSnapshots) ? all.portfolioSnapshots : [],
          portfolioBacktests: isArray(all.portfolioBacktests) ? all.portfolioBacktests : [],
  ...
  useEffect(() => {
    const keys = new Set([
      ...
      'customSettings',
      'fundDailyEarnings',
      ...Object.keys(PORTFOLIO_SYNC_DEFAULTS)
    ]);
    const onStorage = (e) => {
      if (!e.key) return;
    ...
    async (cloudData, cloudUpdatedAt, options = {}) => {
      ...
      try {
        ...
        if (hasOwn(cloudData, 'portfolios')) {
          store.setPortfolios(isArray(cloudData.portfolios) ? cloudData.portfolios : []);
        }
        if (hasOwn(cloudData, 'portfolioHoldings')) {
          store.setPortfolioHoldings(isArray(cloudData.portfolioHoldings) ? cloudData.portfolioHoldings : []);
        }
        if (hasOwn(cloudData, 'portfolioTransactions')) {
          store.setPortfolioTransactions(
            isArray(cloudData.portfolioTransactions) ? cloudData.portfolioTransactions : []
          );
        }
        if (hasOwn(cloudData, 'portfolioPrincipalRecords')) {
          store.setPortfolioPrincipalRecords(
            isArray(cloudData.portfolioPrincipalRecords) ? cloudData.portfolioPrincipalRecords : []
          );
        }
        if (hasOwn(cloudData, 'portfolioSnapshots')) {
          store.setPortfolioSnapshots(isArray(cloudData.portfolioSnapshots) ? cloudData.portfolioSnapshots : []);
        }
        if (hasOwn(cloudData, 'portfolioBacktests')) {
          store.setPortfolioBacktests(isArray(cloudData.portfolioBacktests) ? cloudData.portfolioBacktests : []);
        }
        if (hasOwn(cloudData, 'portfolioSettings')) {
          store.setPortfolioSettings(isPlainObject(cloudData.portfolioSettings) ? cloudData.portfolioSettings : {});
        }
        if (hasOwn(cloudData, 'portfolioSchemaVersion')) {
          store.setPortfolioSchemaVersion(Number(cloudData.portfolioSchemaVersion) || 1);
        }
