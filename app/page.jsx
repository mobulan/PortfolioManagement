...
export default function HomePage() {
  ...
  const handleImportFileChange = async (e) => {
    try {
      ...
      if (isPlainObject(data)) {
        ...
        if (isArray(data.portfolios)) setPortfolios(data.portfolios);
        if (isArray(data.portfolioHoldings)) setPortfolioHoldings(data.portfolioHoldings);
        if (isArray(data.portfolioTransactions)) setPortfolioTransactions(data.portfolioTransactions);
        if (isArray(data.portfolioPrincipalRecords)) setPortfolioPrincipalRecords(data.portfolioPrincipalRecords);
        if (isArray(data.portfolioSnapshots)) setPortfolioSnapshots(data.portfolioSnapshots);
        if (isArray(data.portfolioBacktests)) setPortfolioBacktests(data.portfolioBacktests);
        if (isPlainObject(data.portfolioSettings)) setPortfolioSettings(data.portfolioSettings);
        if (Number.isFinite(Number(data.portfolioSchemaVersion))) {
          setPortfolioSchemaVersion(Number(data.portfolioSchemaVersion));
        }
  ...
  const containerClassName = [
    ...
    isMobile && (mainTab === 'home' || mainTab === 'portfolio') ? 'content-with-mobile-tabbar' : ''
  ]
    .filter(Boolean)
    .join(' ');
  ...
  const mobilePortfolioTabVisible = isMobile && mainTab === 'portfolio';
